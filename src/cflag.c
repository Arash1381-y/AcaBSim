...
#if defined(linux) || defined(__linux) || defined(__linux__) || defined(__OpenBSD__) || defined(__FreeBSD__)           \
    || defined(__NetBSD__)
...
find_short (const struct cflag specs[], int letter)
{
...
static const struct cflag *
find_long (const struct cflag specs[], const char *name)
{
  for (unsigned i = 0; specs[i].name != NULL || specs[i].letter != '\0'; ++i)
    {
      if (!specs[i].name)
...
void
cflag_usage (const struct cflag specs[], const char *progname, const char *usage, FILE *out)
{
  assert (specs);
  assert (progname);
  assert (usage);
...
int
cflag_parse (const struct cflag specs[], int *pargc, char ***pargv)
{
  assert (specs);
  assert (pargc);
  assert (pargv);
...
cflag_status_name (enum cflag_status value)
{
  switch (value)
    ...
    case CFLAG_UNDEFINED:
      return "no such option";
    case CFLAG_BAD_FORMAT:
      return "argument has invalid format";
    case CFLAG_NEEDS_ARG:
...
const char *
cflag_apply (const struct cflag specs[], const char *syntax, int *pargc, char ***pargv)
{
  assert (specs);
  assert (syntax);
  assert (pargc);
...
enum cflag_status
cflag_bool (const struct cflag *spec, const char *arg)
{
  (void)arg;
...
enum cflag_status
cflag_int (const struct cflag *spec, const char *arg)
{
  if (!spec)
    return CFLAG_NEEDS_ARG;
...
enum cflag_status
cflag_uint (const struct cflag *spec, const char *arg)
{
  if (!spec)
    return CFLAG_NEEDS_ARG;
...
enum cflag_status
cflag_float (const struct cflag *spec, const char *arg)
{
  if (!spec)
    return CFLAG_NEEDS_ARG;
...
enum cflag_status
cflag_double (const struct cflag *spec, const char *arg)
{
  if (!spec)
    return CFLAG_NEEDS_ARG;
...
enum cflag_status
cflag_string (const struct cflag *spec, const char *arg)
{
  if (!spec)
    return CFLAG_NEEDS_ARG;
...
enum cflag_status
cflag_bytes (const struct cflag *spec, const char *arg)
{
  if (!spec)
    return CFLAG_NEEDS_ARG;
...
enum cflag_status
cflag_timei (const struct cflag *spec, const char *arg)
{
  if (!spec)
    return CFLAG_NEEDS_ARG;
...
enum cflag_status
cflag_help (const struct cflag *spec, const char *arg)
{
  (void)spec;
  (void)arg;
